...
###################################################################################################################
all_cold_mrs_df <- read.table("../Results/Paper_Text/All_MRS_INV_Low_Supplementary_Table_S7d.csv",sep="&",header=TRUE)
all_cold_mrs_df <- all_cold_mrs_df[all_cold_mrs_df$Median_INV_Enabled>=0 & all_cold_mrs_df$Median_INV_Disabled<=0,]
all_cold_mrs <- as.character(as.vector(all_cold_mrs_df[,1]))

all_hot_mrs_df <- read.table("../Results/Paper_Text/All_MRS_INV_High_Supplementary_Table_S7c.csv",sep="&",header=TRUE)
all_hot_mrs_df <- all_hot_mrs_df[all_hot_mrs_df$Median_INV_Enabled<=0 & all_hot_mrs_df$Median_INV_Disabled>=0,]
all_hot_mrs <- as.character(as.vector(all_hot_mrs_df[,1]))
all_mrs <- union(all_hot_mrs,all_cold_mrs)

RGBM_path <- c("../Data/");

inv_enabled_cancers <- c("BLCA","COAD","LGG","OV","PRAD","PAAD","STAD","LUAD","LUSC","ESCA")
inv_disabled_cancers <- c("HNSC","DLBC","LIHC","THCA")

cancers <- c(inv_disabled_cancers,inv_enabled_cancers)
...
inv_type <- c("INV Enabled","INV Enabled","INV Neutral","INV Neutral","INV Enabled","INV Neutral","INV Neutral","INV Enabled")
...
pdf("../Results/Paper_Figures/Figure7/Heatmap_All_MRs_PRECOG_Figure_7B.pdf",height = 13, width=15, pointsize = 14)
par(bg="white")
...
write.table(activity_df,"../Results/Paper_Text/Diff_MRS_Activity_PRECOG_Table_S9.csv",row.names=T,col.names=T,quote=F,sep=",")
