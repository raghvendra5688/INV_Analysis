...
common_mrs_enabled_list <- c(output_df[output_df$Cancer==INV_enabled[1],]$MR)
for (i in 2:length(INV_enabled))
{
  new_mrs <- output_df[output_df$Cancer==INV_enabled[i],]$MR
...
#Figure 3A 
#=========================================================================================================
pdf("../Results/Paper_Figures/Figure3/Common_TopMRs_Activity_INV_Enabled_Figure_3A.pdf",height = 10, width=12, pointsize = 14)
heatmap.3(common_mrs_enabled_activity_matrix, Rowv = TRUE, Colv=TRUE, col = bluered(100), scale="none", main= "Median Activity of MRs common across all INV Enabled Cancers",
...
#Supplementary information about top common MRs for INV ENabled cancer based on fold change values
A <- common_mrs_enabled_activity_matrix[common_mrs_enabled_list,c(1:length(INV_enabled))];
B <- common_mrs_enabled_activity_matrix[common_mrs_enabled_list,c((length(INV_enabled)+1):(2*length(INV_enabled)))]
...
p3 <- ggplot(data = enabled_activity_df, aes(x=TopMR, y=Activity_Value)) + 
  ...
  ggtitle("Activities of Top 10 MRs common to INV Enabled Cancers ") + theme(text = element_text(size=12)) + theme(plot.title = element_text(hjust = 0.5))

ggsave(file="../Results/Paper_Figures/Figure3/Common_TopMRs_Activity_Figure_3C.pdf",plot = p3, device = pdf(), width = 12, height=12, units = "in", dpi = 300)
dev.off()

#Perform the same analysis for INV Disabled cancers
#==============================================================================================================================
common_mrs_disabled_list <- c(output_df[output_df$Cancer==INV_disabled[1],]$MR)
for (i in 2:length(INV_disabled))
{
  new_mrs <- output_df[output_df$Cancer==INV_disabled[i],]$MR
...
#Figure 3B
#=========================================================================================================
pdf("../Results/Paper_Figures/Figure3/Common_TopMRs_Activity_INV_Disabled_Figure_3B.pdf",height = 10, width=12, pointsize = 12)
heatmap.3(common_mrs_disabled_activity_matrix, Rowv = TRUE, Colv=TRUE, col = bluered(100), scale="none", main= "Median Activity of MRs common across all INV Disabled Cancers",
...
#Supplementary information about top common MRs for INV Disabled cancer based on fold change values
A <- common_mrs_disabled_activity_matrix[common_mrs_disabled_list,c(1:length(INV_disabled))];
B <- common_mrs_disabled_activity_matrix[common_mrs_disabled_list,c((length(INV_disabled)+1):(2*length(INV_disabled)))]
wilcox_test_info <- perform_wilcox_test(A,B)
wilcox_test_info <- wilcox_test_info[order(wilcox_test_info$FC_Mean,decreasing=TRUE),]
wilcox_test_info <- revise_wilcox_test_info(wilcox_test_info)
write.table(wilcox_test_info,"../Results/Paper_Text/Supplementary_Table_S2_Common_MRs_INV_Disabled.csv",row.names=T,col.names=T,quote=F)
...
p4 <- ggplot(data = disabled_activity_df, aes(x=TopMR, y=Activity_Value)) + 
  ...
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + theme(axis.text.x = element_text(angle = -90)) +
  ggtitle("Activities of Top 10 MRs common to INV Disabled Cancers ") + theme(text = element_text(size=12)) + theme(plot.title = element_text(hjust = 0.5))

ggsave(file="../Results/Paper_Figures/Figure3/Common_TopMRs_Activity_Figure_3D.pdf",plot = p4, device = pdf(), width = 12, height=12, units = "in", dpi = 300)
dev.off()
